...
public class HttpUtils {
     ...
     * @return
     */
    public static <T> T post(String url, Object obj, Class<T> cla) {
        return post(url, obj, cla, null);
    }

    public static <T> T post(String url, Object obj, Class<T> cla, String token) {
        ...
        FastJsonHttpMessageConverter m = new FastJsonHttpMessageConverter();
        restTemplate = new RestTemplateBuilder().additionalMessageConverters(m).build();
        HttpEntity<T> formEntity = new HttpEntity(obj, requestHeaders);
        T aaa = restTemplate.postForObject(url, formEntity, cla);
        return aaa;
    }
    ...
    public static String postXml(String path, String xml, String encoding) throws Exception {
        ...
        if (conn.getResponseCode() == 200) {
            return new String(read2Byte(conn.getInputStream()));
        }
        return null;
    }
    ...
    public static String postJson(String path, String json) throws Exception {
        byte[] data = json == null ? "".getBytes(defCharset) : json.getBytes(defCharset);
        URL url = new URL(path);
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("POST");
        conn.setDoOutput(true);
        ...
        if (conn.getResponseCode() == 200) {
            ...
            return new String(read2Byte, defCharset);
        } else {
